...
- **3点**: 概ね適切だが、一部のタスクでモデルの選定がタスク難易度と不整合
  （定型処理を上位モデルで実行等）。`cache_hit_pct` がやや低い、または
  `compactions` が数回発生している。
- **1点**: ほぼ全タスクを最上位モデル（Opus/Fable）で実行、または委譲プロンプト
  なしにサブエージェントを暴走させている（102 agents事例・高effort暴走事例に
  類する兆候）。`compactions` が頻発（3回超）し1セッションへの詰め込みが常態化。
