...
- **T4実施結果（2026-07-11 13:15 UTC、当初予定通りフォローアップRoutineで実行）**:
  軽量クエリ1本を投げ `status: ok`（16秒）で成功。直前の復旧確認
  （2026-07-11 00:28 UTC）から**12時間47分経過・その間クエリ実行ゼロ**でも
  失効しなかった。
  - これは「絶対期限説」への反証データになる: もし単純な固定時間（例:
    前回の失効が10時間程度で起きたことから類推される閾値）でグラントが
    切れるなら、12時間47分経過した今回はとっくに失敗しているはず。
  - 従って**当初の仮説（H2: 複数回の独立したrefresh実行がreuse判定された）
    の方が有力に戻った**と判断する。前回の失効は「T0/T2/T3/T6を約12分間に
    4回連続実行し、その都度独立してrefresh tokenの交換を行った」という
    テスト特有の連投パターンが原因で、**通常の間隔を空けた利用では
    起きにくい**可能性が高い。ただし1回のデータ点なので確定ではない。
  - 運用上の結論は変わらず: 短時間に複数クエリを連投するテスト・デバッグ時は
    失効リスクが上がることを認識した上で行う。通常利用（数十分〜数時間おき）
    では今のところ問題が再現していない。C1着手条件（上記）は据え置き。
