...
| 項目 | 値 |
|---|---|
| ステータス | レビュー済み（red-team 3指摘を全採択・反映済み。実装は §12 のユーザー回答待ち） |
| 種別 | パイプライン |
| 優先度 | Tier F-1（収益直結） |
...
**調査経路の記録**: hermes-relay へ x_search / web_search の2クエリを送信したが、両方とも `xAI token refresh failed (invalid_grant: Refresh token has been revoked)` で実行失敗（2026-07-11。実行基盤の GitHub Actions は正常、Grok OAuth が失効）。X 一次ポストの調査は再認証後に再送する（§12-6）。以下はユーザー指示に基づく Haiku + WebSearch の代替調査結果（二次情報中心・未検証値を含む）。
...
**プラットフォーム推奨（本設計の結論）**: 主経路=クラウドワークス（案件数最多・AI案件急増・初実績づくりに最適）、並走=ココナラ出品（待ち受け・規約リスク構造的に低い）、ランサーズは CW と同型なので通知監視だけ相乗り。海外（Fiverr/Upwork）は Phase 3 以降の拡張候補（§12-7）。
...
| 2 | **規約違反疑いでアカウント制限**（自動化がバレる/疑われる） | プラットフォームからの警告メール・提案の非表示化 | 自動化は「メール受信とローカル生成」に限定し、プラットフォーム上の操作は全て人間（G1〜G4）。スクレイピング恒久禁止（§2.2）。提案文の透明性1文で AI 利用を隠さない |
...
6. **hermes-relay の再認証**: Grok OAuth が失効中（§0.4）。ユーザーPCで `hermes auth add xai-oauth` を再実行し、`HERMES_SEED_B64` シークレットを再作成後、X 一次ポストの事例調査を再送するか
