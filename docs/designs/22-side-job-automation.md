...
**§13.9・§13.11 による訂正**: 上記1・2・3の `pipeline-state.md`・`proposals/`・`receipts/` は、実装当初このブランチ直下に置いていたが、実運用で実データ（クライアント名・単価・案件URL）を持つことが判明したため分離した。**最終的な格納先は独立した private リポジトリ `ryotaroh180105/sidejob-ledger`**（§13.11。当初 `sidejob-ledger` ブランチに分離したが、publicリポジトリの別ブランチは非公開にならないと判明し是正）。DoDの検証手順自体は変わらない（`sidejob-ledger` リポジトリを clone した上で同じ手順を実行する）。

## 4. 成果物の構成（ファイルレイアウト）

**構造とデータを分離する（§13.9・§13.11 で確定）**: 案件台帳・提案文・受入監査ログは実運用が始まると実際のクライアント名・単価・案件URLを含む「実データ」になる。README「配置規約」F区分（実データはコミット禁止・ローカル or private リポジトリ）に従い、以下のとおり分ける。

**`claude-skills` リポジトリ（構造のみ。public）**:
...
**`ryotaroh180105/sidejob-ledger`（独立した private リポジトリ。実データ）**:
...
4. ~~Routine 登録の承認~~ → **intakeのみ解決済み**（2026-07-12、ユーザー「君がトリガーで察知してよ」を承認として登録。trigger_id: trig_015WYXio1H7CWFirVMa7UrzA、毎日JST朝7時。詳細は `loops/sidejob-intake/schedule.md`）。weekly-review・expansion-reviewのRoutine登録は引き続き未承認・未登録
...
## 13.10 拡張候補1（SNS運用代行）の採択・実装（2026-07-11）

§13.8 の候補1（SNS運用代行）をユーザーが採択。候補2（業務自動化・AIエージェント構築の受託）は §13.8 で「専用スキル無し・テンプレ化しにくい」と判定済みのため、パターンが複数観測されるまで実装しない（yagni方針。§2.2と同じ考え方）。候補3（AI占い等）は非推奨のため実装しない。

### 実装内容

- `sidejob/config.md` の対象カテゴリ表に「SNS運用代行」を追加。Doerは既存の `sns-ops-team`（企画・執筆・レビュー）+ `sns-auto-posting`（投稿文・スケジュール生成）。新規Doerスキルの作成は不要（§13.8の判断どおり）
- 単価下限は個人向け月額10,000円・法人向け月額50,000円（2026-07-11時点の市場調査に基づく暫定値、未検証。実データ蓄積後に weekly-review で見直す）
- **スコープ限定（新規の設計判断）**: 納品は投稿文・画像案・投稿スケジュール表までとし、クライアントのSNSアカウントへの実投稿は行わない。理由: `sns-auto-posting` はRyo自身のアカウント用にOAuth認証されたツールであり、クライアントのアカウントに実投稿するには当該アカウントのOAuth/APIトークンを預かる必要が生じる。これはRyo自身の運用（設計書全体が前提とする信頼境界）とは異なる第三者認証情報の取り扱いになるため、初期スコープでは意図的に除外する
- **月額契約（recurring）の台帳運用**: `pipeline-state.md` のスキーマ（§5.2）は変更しない。月ごとに新しい行を追加し、notes列で同一クライアントの継続契約であることを明記する運用とする（新しいstatus値・列を追加しない。yagni）
- Lane構成（A/B/C/D）自体の拡張はしない。SNS運用代行の案件発見は既存のLane B intakeループ（CW/ランサーズの通知メール）にそのまま乗る（両プラットフォームともSNS運用代行の案件カテゴリが存在するため、Gmail検索クエリ・差出人検疫の変更は不要）

### やらないこと（このタイミングでは実装しない）

- 業務自動化・AIエージェント構築の受託のDoerスキル化（§13.8の判断を維持。パターン観測が先）
- クライアントアカウントへの実投稿の自動化（上記スコープ限定により恒久的に対象外ではなく、将来クライアントから明示的にアカウントアクセスを付与された場合は個別に再検討する）
- 新しいLane（E等）の新設（月額契約は既存スキーマの運用上の工夫で対応できるため不要と判断）

### 反映

- `sidejob-ledger` の `expansion-candidates.md` に候補1を「採択」として追記済み（当時は `claude-skills` の `sidejob-ledger` ブランチ。§13.11の是正後、private リポジトリへ移設済み）

## 13.11 再是正: 実データ格納先を private リポジトリへ移行（2026-07-11）

§13.9でユーザーGmail情報（実際の受信箱の検索結果）を `sidejob-ledger` ブランチへコミット・pushしようとしたところ、Claude Code のauto-mode分類器が「publicリポジトリへの実データ公開」としてブロックした。これにより §13.9 の是正が**不十分**だったことが判明した: `claude-skills` はpublicリポジトリであり、別ブランチに分けても GitHub 上では誰でも閲覧可能なままで、README「配置規約」F区分の「private リポジトリ」の要件を満たしていなかった。

**是正内容**: ユーザーが新規に独立したリポジトリ `ryotaroh180105/sidejob-ledger` を private 設定で作成。Claude が `list_repos` で `visibility: private` を確認した上で、旧 `claude-skills` の `sidejob-ledger` ブランチの内容（§13.9時点では全て創作サンプル・Claude自身の分析であり実クライアントデータは含まれていなかった）を移設し、加えてGmail実データ（送信元・件名・受信日時）を含む初回実行記録をこの private リポジトリにのみ記録した。

**気づいた経緯**: ブロックされたことで気づいた。ブロックされていなければ、実際のGmailメタデータが public リポジトリの `sidejob-ledger` ブランチに公開されるところだった（実害は発生していない。push前に分類器がブロックしたため）。教訓: 「別ブランチに分離する」＝「非公開になる」ではない。visibility（可視性）の実際の設定を確認せずに「分離したから安全」と判断したのが根本原因。今後、実データの格納先を新設・変更する際は、**格納先の実際のvisibilityを都度確認してから push する**手順を CONTRACT.md に明記した（§6.2 データの読み書き手順・手順4）。

**旧 `claude-skills` の `sidejob-ledger` ブランチ（public）の扱い**: 実クライアントデータは含まれていない（架空データ・Claudeの分析のみ）ため緊急の実害はないが、以後の実データはこのブランチに一切記録しない。ブランチ自体の削除は破壊的操作のためユーザー判断待ち（§12関連）。
