...
read.amazon.co.jp/notebook からの貼り付け＝14 §6.2 経路2は補助経路として許容。
「法的リスクで却下」されたのはブラウザ拡張による自動読取であり、貼り付けではない
——2026-07-07 レビューで本メモの誤記を訂正）であり、本節の案A/B とは
...
- ~~**設計14が存在しない**~~（解消済み・2026-07-07）: 執筆時は並列設計中で14が見えなかったが、14は完成・実装済み。Kindle入力は14のMy Clippings.txtパーサ方式が正、本書§6.4の案A/Bは前段構想（冒頭の設計整合メモ参照）。
